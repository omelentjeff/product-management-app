...
-- Create the `nutritional_fact` table with UTF-8 encoding
...
-- Create the `product` table with UTF-8 encoding
CREATE TABLE IF NOT EXISTS `product` (
    `id` BIGINT PRIMARY KEY AUTO_INCREMENT,
    `manufacturer` VARCHAR(255) CHARACTER SET utf8 COLLATE utf8_general_ci,
    `name` VARCHAR(255) CHARACTER SET utf8 COLLATE utf8_general_ci,
    `weight` DECIMAL(10, 2),
    `nutritional_fact_id` BIGINT,
    `photo_url` VARCHAR(255) CHARACTER SET utf8 COLLATE utf8_general_ci,
    `gtin` VARCHAR(50) CHARACTER SET utf8 COLLATE utf8_general_ci,
    CONSTRAINT `fk_product_nutritional_fact` FOREIGN KEY (`nutritional_fact_id`) REFERENCES `nutritional_fact` (`id`) ON DELETE CASCADE
) CHARACTER SET utf8 COLLATE utf8_general_ci;

-- Create the `user_entity` table with UTF-8 encoding
...
ALTER DATABASE products_db CHARACTER SET utf8 COLLATE utf8_general_ci;

-- Insert sample data into `nutritional_fact` table
...
(274, 1140, 4.4, 93, 0.11, 88, 0.94, 0, 0.03);
